...
### List all spaces
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env - <<'EOF' | jq '.results[] | {id, key, name, type}'
...
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable space_key="<SPACE_KEY>" - <<'EOF' | jq '.results[0] | {id, key, name}'
GET {{base_url_confluence}}/api/v2/spaces
[QueryStringParams]
keys: {{space_key}}
[Options]
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable page_id="<page_id>" - <<'EOF' | jq '{id, title, version: .version.number, spaceId, body: .body.storage.value}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable parent_id="<parent_id>" - <<'EOF' | jq '.results[] | {id, title}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable page_id="<page_id>" - <<'EOF' | jq '.results[] | {id, title}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable space_id="<space_id>" - <<'EOF' | jq '{results: [.results[] | {id, title}], next: ._links.next}'
GET {{base_url_confluence}}/api/v2/pages
[QueryStringParams]
spaceId: {{space_id}}
limit: 50
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable cql="<query>" - <<'EOF' | jq '.results[] | {id, title}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable page_id="<page_id>" - <<'EOF' | jq '.results[] | {id, author: .author.displayName, body: .body.storage.value}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable page_id="<page_id>" - <<'EOF' | jq '.results[] | {id, author: .author.displayName, status, body: .body.storage.value}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable comment_id="<comment_id>" - <<'EOF' | jq '{id, author: .author.displayName, body: .body.storage.value}'
...
```bash
hurl --variables-file ~/.config/hurl/confluence/default.env \
  --variable page_id="<page_id>" - <<'EOF' | jq '.results[] | {id, name}'
